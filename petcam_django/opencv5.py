...
while True:
    ...
    if key == ord('r') and is_record == False:  # 현재 녹화상태가 아니며 r 키를 누르면
        ...
        video = cv2.VideoWriter("media/video/웹캠 " + nowDatetime_path + ".avi", fourcc, 15, (frame.shape[1], frame.shape[0]))
        ...
        cv2.imwrite("media/capture/capture " + nowDatetime_path + ".png", frame)  # 파일이름(한글안됨), 이미지 
